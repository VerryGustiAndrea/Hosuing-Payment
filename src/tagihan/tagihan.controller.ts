...
export class TagihanController {
  ...
  @Post('approval/:id')
  async approval(@Query() query: Record<string, any>, @Query('trx_id') trx_id: number, @Query('sid') sid: string, @Query('status') status: string, @Query('via') via: string, @Param('id') id: number) {
    try {
